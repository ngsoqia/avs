    ...
    if ( isset($_POST['edit_user']) ) {
        ...
        if ( $_FILES['avatar']['tmp_name'] != '' && !$errors ) {
			...
			if (!$errors) {
				...
				$dst_tmp	= $config['BASE_DIR']. '/tmp/avatars/'.$id.'.'.$ext;
				if (move_uploaded_file($src, $dst_tmp)) {
					require $config['BASE_DIR']. '/classes/image.class.php';
					$dst_orig	= $config['BASE_DIR']. '/media/users/orig/'.$id.'.jpg';
					...
					$dst	= $config['BASE_DIR']. '/media/users/'.$id.'.jpg';				
        ...
        if ( !$errors ) {
            ...
            if ( isset($photo_new) ) {
                $sql_add .= " ,photo = '" .mysql_real_escape_string($id.'.jpg'). "'";
            }
            ...
            $sql = "UPDATE signup SET premium = '".mysql_real_escape_string($_POST['premium'])."' and email = '" .mysql_real_escape_string($email). "', fname = '" .mysql_real_escape_string($fname). "',
                                      ...
                                      account_status = '" .mysql_real_escape_string($account_status). "'" .$sql_add. " WHERE UID = '" .mysql_real_escape_string($id). "' LIMIT 1";
            $conn->execute($sql); 
